...
from tarp.functional.padding.convolution import Autopad, PaddingMode, autopad_nd


class FastConvolutionLayer1D(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
        kernel_size: int,
        stride: int = 1,
        padding: int | Autopad = Autopad.VALID,
        dilation: int = 1,
        groups: int = 1,
        bias: bool = True,
    ...
    def reset_parameters(self):
        if self.weight is not None:
            _ = nn.init.kaiming_uniform_(self.weight, a=math.sqrt(5))
        ...
        if self.bias is not None:
            ...
            _ = nn.init.uniform_(self.bias, -bound, bound)
    ...
    @override
    def forward(self, input: Tensor, external_weight: Tensor | None = None) -> Tensor:
        """
